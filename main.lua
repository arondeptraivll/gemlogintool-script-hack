...
-- Bọc toàn bộ script trong một pcall để bắt bất kỳ lỗi runtime nào
local success, errorMessage = pcall(function()
    
    -- Xóa các GUI cũ để tránh trùng lặp
    ...
    pcall(function() workspace:FindFirstChild("GemLogin_ESP_Container"):Destroy() end) -- Xóa container ESP cũ trong workspace

    -- === KHAI BÁO CÁC DỊCH VỤ VÀ BIẾN ===
    ...
    local isLineEspEnabled, isDistanceEspEnabled = false, false
    local espRenderObjects = {} -- Lưu các đối tượng trực quan cho ESP
    local espConnections = { Player = {}, Character = {} } -- Lưu các kết nối sự kiện cho ESP
    ...
    -- [SỬA LỖI QUAN TRỌNG] Đặt Container trong WORKSPACE thay vì CoreGui để các Part có thể hiển thị.
    local ESPContainer = Instance.new("Folder", workspace)
    ESPContainer.Name = "GemLogin_ESP_Container"

    -- [Cấu trúc GUI được giữ nguyên]
    ...
    -- === LOGIC ESP VIẾT LẠI (Line/Distance) ===
    local function createDistanceGuiForCharacter(player)
        if not espRenderObjects[player] then return end
        
        -- Dọn dẹp GUI cũ nếu có
        if espRenderObjects[player].distance then
            espRenderObjects[player].distance:Destroy()
        end
    ...
    local function setupPlayerForEsp(player)
        ...
        -- Line
        local line = Instance.new("Part", ESPContainer)
        ...
        line.Size = Vector3.zero -- Ẩn ban đầu
        objects.line = line

        espRenderObjects[player] = objects

        -- Character Added/Respawn Logic
        espConnections.Character[player.UserId] = player.CharacterAdded:Connect(function()
            createDistanceGuiForCharacter(player)
        end)
        
        createDistanceGuiForCharacter(player) -- Chạy lần đầu
    end
    ...
    local function updateEspVisuals()
        ...
        for player, objects in pairs(espRenderObjects) do
            ...
            if targetRoot and targetChar.Humanoid.Health > 0 then
                ...
                if isLineEspEnabled and objects.line then
                    objects.line.Size = Vector3.zero -- Ẩn nếu mục tiêu không hợp lệ
                end
            end
        end
    end
    ...
    function toggleLineEsp(enabled)
        ...
        if not enabled then
            for _, objects in pairs(espRenderObjects) do
                if objects.line then objects.line.Size = Vector3.zero end
            end
        end
        toggleEspLoop(isLineEspEnabled or isDistanceEspEnabled)
    end
    ...
    -- Xử lý người chơi ra/vào cho tất cả các chức năng
    ...
    -- Setup ban đầu cho những người chơi đã có trong server
    ...
    game:GetService("CoreGui").gemlogin_tool.Destroying:Connect(function()
        ...
        for _, conn in pairs(espHighlightConnections) do if type(conn) == "RBXScriptConnection" then conn:Disconnect() end end
...
if not success then
    ...
    print("gemlogin tool by TuanHaii (v10.3 - Definitive Edition+) đã được tải thành công!")
end
